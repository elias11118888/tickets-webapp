  ...
  const checkUserRole = async () => {
    try {
      ...
      if (data.role) {
        ...
        // Check if this could be the first user
        const initResponse = await fetch("/api/admin/initialize-super-admin", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
        });
        ...
        if (initData.initialized) {
          // Successfully initialized as super admin
          window.location.href = "/admin/dashboard";
          return;
        } else if (
          !initData.initialized &&
          initData.message === "Super admin already exists"
        ) {
          // Not an admin, show access denied
          setUserRole("denied");
        } else {
          // Could be first user, redirect to initialization
          window.location.href = "/admin/initialize";
          return;
        }
      }
    } catch (error) {
